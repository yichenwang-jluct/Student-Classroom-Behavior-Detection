...
class BboxLoss(nn.Module):
    ...
    def _df_interval(self):
        ...
        t = max(0.0, min(1.0, DFState.epoch / max(1, DFState.epochs)))
        d = self.df_d0 * t
        u = 1.0 - (1.0 - self.df_u0) * t
        if getattr(self, "_probe_last", -1) != DFState.epoch:
            self._probe_last = DFState.epoch
            print(f"[DF-PROBE] epoch={DFState.epoch}/{DFState.epochs}  t={t:.4f}  d={d:.4f}  u={u:.4f}", flush=True)
        return d, u
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask, hw=None):
        ...
        if self.use_df_ciou:
            ...
            d, u = self._df_interval()
            iou_df = ((iou_plain - d) / (u - d + 1e-7)).clamp(0.0, 1.0)   # Eq. 6
            penalty = iou_plain - iou                                     # CIoU geometric term
            loss_box = (1.0 - iou_df) + penalty                          # Eq. 7
...
class v8DetectionLoss:
    ...
    def __init__(self, model):  # model must be de-paralleled
        ...
        "Uncomment a line below to switch the classification loss; the default is the standard one."
        ...
        # self.bce = QualityfocalLoss()  # detection only; not supported for segmentation or pose
        self.hyp = h
    ...
    def __call__(self, preds, batch):
        ...
        if isinstance(self.bce, (nn.BCEWithLogitsLoss, Vari_focalLoss, Focal_Loss)):
            ...
            loss[1] = self.bce(pred_scores, target_scores.to(dtype)).sum() / target_scores_sum  # keep a usable loss term
